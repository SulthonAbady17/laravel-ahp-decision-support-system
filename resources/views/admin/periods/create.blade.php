            ...
            <x-card>
                ...
                <div class="space-y-4">
                    <div class="flex flex-col gap-1">
                        ...
                        <x-form.input :value="old('name')" autofocus id="name" name="name"
                            placeholder="Contoh: Pemilihan Ketua 2025" required type="text" />
                    </div>

                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div class="flex flex-col gap-1">
                            <x-form.label for="start_date" value="Tanggal Mulai" />
                            <x-form.input :value="old('start_date')" id="start_date" name="start_date" type="datetime-local" />
                        </div>
                        <div class="flex flex-col gap-1">
                            <x-form.label for="end_date" value="Tanggal Selesai" />
                            <x-form.input :value="old('end_date')" id="end_date" name="end_date" type="datetime-local" />
                        </div>
                    </div>

                    <div class="flex flex-col gap-1">
                        <x-form.label for="status" value="Status" />
                        <x-form.combobox :options="$statusOptions" :value="old('status')" id="status" name="status"
                ...
                <x-slot name="footer">
                    <div class="flex items-center justify-end">
                        <x-button-link href="{{ route('admin.periods.index') }}" variant="outline">Batal</x-button-link>
