...
<div class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0" style="display: {{ $show ? 'block' : 'none' }};"
    x-data="{ show: @js($show) }" x-on:close-modal.window="$event.detail === '{{ $name }}' ? show = false : null"
    ...
    {{-- Konten Modal --}}
    <div class="{{ $maxWidthClasses }} mb-6 transform overflow-hidden rounded-lg bg-white shadow-xl transition-all sm:mx-auto sm:w-full"
