<nav aria-label="Main navigation"
    ...
    <div class="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <ul class="hidden items-center gap-6 sm:flex">
            @auth
                ...
                @if (Auth::user()->role === 'admin')
                    ...
                    <li><x-link :route="'admin.users.index'">Users</x-link></li> {{-- <-- Link Baru --}}
                @endif
        ...
        <ul class="divide-outline rounded-b-radius border-outline bg-surface-alt dark:divide-outline-dark dark:border-outline-dark dark:bg-surface-dark-alt fixed inset-x-0 top-0 z-10 flex max-h-svh flex-col divide-y overflow-y-auto border-b px-6 pb-6 pt-20 sm:hidden"
            ...
            @auth
                ...
                @if (Auth::user()->role === 'admin')
                    ...
                    <li class="py-4"><a
                            class="text-on-surface dark:text-on-surface-dark w-full text-lg font-medium focus:underline"
                            href="{{ route('admin.users.index') }}">Users</a></li> {{-- <-- Link Baru --}}
                @endif
                <li class="py-4"><a
                        class="text-on-surface dark:text-on-surface-dark w-full text-lg font-medium focus:underline"
